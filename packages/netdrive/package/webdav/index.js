const { WebDAVServer } = require('@netdrive/webdav')
...
const isWebDAVRequest = (ctx, webdavPath) => {
  if (webdavPath == '/') {
    return /(Microsoft\-WebDAV|FileExplorer|WinSCP|WebDAVLib|WebDAVFS|rclone|Kodi|davfs2|netdrive\-webdav|RaiDrive|nPlayer|LibVLC|PotPlayer|gvfs)/i.test(ctx.request.headers['user-agent']) || ('translate' in ctx.request.headers) || ('overwrite' in ctx.request.headers) || ('depth' in ctx.request.headers)
  } else {
