...
const factory = require('@netdrive/core')
...
module.exports = (app) => {
  app.addSingleton('netdrive', async (options) => {
    ...
    const getDisk = () => { return config.drives.map((i) => ({ name: i.name, path: driver.decode(i.path) })) }
    const setDisk = (data) => {
      data.forEach((i) => {
