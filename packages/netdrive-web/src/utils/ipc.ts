...
const setMode = (v: string): void => {ipcRenderer.send('set_mode', v)}
const setServer = (data: string): void => { ipcRenderer.send('set_server', data)}
const getMode = (): void => { ipcRenderer.sendSync('get_mode')}
